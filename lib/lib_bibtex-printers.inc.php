...
abstract class BasePrinter {
	...
	private function GetPaperDirectory() {
		return $this->paperDir;
	}

	private function SetPaperDirectory($dir) {
		$this->paperDir = $dir;
	}
	...
	private function BibliographyEntryStr($entry, $usage) {
	...
	private function StartCiteStr() {
		return '<span class="bibtex-cite">';
	}
	private function EndCiteStr() {
		return '</span>';
	}
	
	private function StartBibliographyEntryStr($entry) {
		$str = "\t" .'<tr id="' . $entry['key'] . '" class="bibtex-entry bibtex-'.$this->GetPrinterName().'">' . "\n";
		return $str;
	}
	private function EndBibliographyEntryStr() {
		$str = "\t" .'</tr>' . "\n";
		return $str;
	}

	private function StartBibliographyRefStr() {
		return "\t" . "\t" .'<td class="bibtex-reference bibtex-'.$this->GetPrinterName().'">';
	}
	private function EndBibliographyRefStr() {
		return '</td>' . "\n";
	}


	private function StartBibliographyCitationStr() {
		return "\t" . "\t" .'<td class="bibtex-citation">';
	}
	private function EndBibliographyCitationStr() {
		return '</td>' . "\n";
	}

	private function RemoveTag($line) {
		$pattern = "/\<span(.*)\'\>|\<\/span\>/i";
		return preg_replace($pattern, '', $line);
	}

	// Gives the string of the full citation, like for in a list
	private function CitationStr($entry) {
		global $Path;
	...
	private function CitationNoteStr($entry) {
		$str = (isset($entry['note']) ? ', <span class="bibtex-note">(' .$entry['note'] . ')</span>.' : '' );
		return $str;
	}
	private function CitationImplementUrlStr($entry) {
		global $Path;
	...
	private function StartCitationStr($entry) {
		$str = '<span class="bibtex-' . $entry['type'] . '">';
		return $str;
	}
	private function EndCitationStr() {
		return '</span>';
	}

	private function AuthorsStr($entry) {
	...
	private function FullNamesStr($author, $maxnum=-1) {
	...
	private function PagesStr($entry) {
	...
	// base ListStartStr()
	private function ListStartStr() {
		return '[';
	}
	// base ListEndStr()
	private function ListEndStr() {
		return ']';
	}
	// base ListSepStr()
	private function ListSepStr() {
		return ',';
	}
	// base BaseUnknownRefStr()
	private function UnknownRefStr() {
		return '?';
	}
