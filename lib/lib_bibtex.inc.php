...
class Bibtex {
	...
	private function SetBibliographyStyle($style) {
		if($style == 'abbrv') {
	...
	//           = usage
	private function SetBibliographyOrder($order) {
		if($order == 'alphabetic') {
	...
	private function Parse($pathname) {
		$file = file($pathname);
		$rawblock = '';
		$inblock = false;
	...
	private function ParseEntry($rb) {
		$rb = trim($rb);
		if($rb == '')
			return;
	...
	private function NormalizeAuthorStr($authorstr) {
	...
	//////////////////////////////////////////////////////////////////////////////
	private function Citep() {
		$args = func_get_args();
		CiteRefs('p', $args);
	}
	private function Citet() {
		$args = func_get_args();
		CiteRefs('t', $args);
	}
	private function CiteRefs($style, $args) {
		$style = $style == 't' ? 't' : 'p';
		$num_args = count($args);
	...
	//////////////////////////////////////////////////////////////////////////////
	private function IncludeBibContent($pathname, $bib = false) {
		if($bib != false)
			eval ('global '.$bib.';');
		Bibtex::ScanContentForCites($pathname, $bib);
		include_once $pathname;
	}
	private function ScanContentForCites($pathname, $bib) {
		if($bib != false)
			eval ('global '.$bib.';');
	...
	private function ResetBibliography() {
		$this->scanning = false;
		$this->scanned = false;
		$this->used = array();
		$this->usedCnt = 0;
	...
	// impl for Bibarray
	private function OrderBibliography() {
	...
	private function CmpEntriesYear($a, $b, $d) {
		if($a['year'] < $b['year'])
			return ($d == 'a' ? -1 : 1);
	...
	// impl for BibArray
	private function Select($queryArray) {
	...
	private function SelectEntry($key, $entry = false) {
	...
	private function SelectEntries() {
		$args = func_get_args();
		$num_args = count($args);
	...
	private function PrintBibliography() {
	...
	private function PrintBibliographyCitedOnly() {
		if($this->scanning === true) return;
		echo '<table class="bibtex-biblio">' . "\n";
		$this->OrderBibliography();	
		foreach($this->used as $key => $info) {
	...
	private function PrintBibliographySelectedOnly() {
		if($this->scanning === true) return;
		echo '<table class="bibtex-biblio">' . "\n";
		$this->OrderBibliography();	
		foreach($this->used as $key => $info) {
